    ...
    <div>
      ...
      <div className ="w-1/2">
        {product.imagems?.map((image, index) => (
          <img className="w-full object-cover rounded-lg hover:scale-105 transition-transform duration-300" src={image} alt={`${index}`} key={index} />
        ))}
      </div>
      ...
      <p className="text-2xl font-bold text-gray-800">Preço: R${product.preco.toFixed(2)}</p>
      </div>
